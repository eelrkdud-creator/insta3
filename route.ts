...
export async function POST(req: Request) {
  ...
  if (!IG_REGEX.test(url)) {
    return NextResponse.json<ApiError>(
      {
        ok: false,
        error: { code: "INVALID_URL", message: "Invalid or unsupported Instagram URL." }
      },
      { status: 400 }
    );
  }
    ...
    return NextResponse.json<ApiError>(
      {
        ...
        error: {
          code: "PARSING_FAILED",
          message: "Unable to read post metadata."
        }
      },
      { status: 422 }
    );
  ...
  if (!uploadedAtUtc) {
    return NextResponse.json<ApiError>(
      {
        ok: false,
        error: { code: "PARSING_FAILED", message: "Missing upload date." }
      },
      { status: 422 }
    );
  }
